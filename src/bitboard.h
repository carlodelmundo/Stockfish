...
inline Bitboard rank_bb(Square s) { return RankBB[rank_of(s)]; }

inline Bitboard file_bb(File f) { return FileBB[f]; }

inline Bitboard file_bb(Square s) { return FileBB[file_of(s)]; }

/// shift() moves a bitboard one step along direction D. Mainly for pawns

template <Square D> inline Bitboard shift(Bitboard b) {
  return D == NORTH ? b << 8
                    : D == SOUTH ? b >> 8
                                 : D == NORTH_EAST
                                       ? (b & ~FileHBB) << 9
...
inline Bitboard adjacent_files_bb(File f) { return AdjacentFilesBB[f]; }
...
/// color. For instance, forward_ranks_bb(BLACK, SQ_D3) will return the 16
...
///      PawnAttackSpan[c][s] = forward_ranks_bb(c, s) &
///      adjacent_files_bb(file_of(s));
...
/// number of steps for a king in x to reach y. Works with squares, ranks,
/// files.
