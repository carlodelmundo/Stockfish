...
ExtMove *generate_castling(const Position &pos, ExtMove *moveList, Color us) {
  ...
  if (Chess960 && (attacks_bb<ROOK>(kto, pos.pieces() ^ rfrom) &
...
ExtMove *generate_pawn_moves(const Position &pos, ExtMove *moveList,
  ...
  Bitboard enemies =
      (Type == EVASIONS ? pos.pieces(Them) & target
  ...
  if (Type != CAPTURES) {
    ...
    if (Type == QUIET_CHECKS) {
      ...
      if (pawnsNotOn7 & dcCandidates) {
        Bitboard dc1 = shift<Up>(pawnsNotOn7 & dcCandidates) & emptySquares &
                       ~file_bb(ksq);
...
ExtMove *generate_all(const Position &pos, ExtMove *moveList, Bitboard target) {
  ...
  if (Type != CAPTURES && Type != EVASIONS && pos.can_castle(Us)) {
    if (pos.is_chess960()) {
      moveList =
          generate_castling<MakeCastling<Us, KING_SIDE>::right, Checks, true>(
              pos, moveList, Us);
      moveList =
          generate_castling<MakeCastling<Us, QUEEN_SIDE>::right, Checks, true>(
              pos, moveList, Us);
    } else {
      moveList =
          generate_castling<MakeCastling<Us, KING_SIDE>::right, Checks, false>(
              pos, moveList, Us);
      moveList =
          generate_castling<MakeCastling<Us, QUEEN_SIDE>::right, Checks, false>(
              pos, moveList, Us);
    }
  }
...
/// underpromotions that give check. Returns a pointer to the end of the move
/// list.
template <>
ExtMove *generate<QUIET_CHECKS>(const Position &pos, ExtMove *moveList) {
  ...
  return us == WHITE
             ? generate_all<WHITE, QUIET_CHECKS>(pos, moveList, ~pos.pieces())
