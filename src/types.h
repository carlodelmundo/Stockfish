...
inline Rank relative_rank(Color c, Rank r) { return Rank(r ^ (c * 7)); }
...
inline Square pawn_push(Color c) { return c == WHITE ? NORTH : SOUTH; }

inline Square from_sq(Move m) { return Square((m >> 6) & 0x3F); }

inline Square to_sq(Move m) { return Square(m & 0x3F); }

inline int from_to(Move m) { return m & 0xFFF; }
...
inline Move make_move(Square from, Square to) { return Move((from << 6) + to); }
