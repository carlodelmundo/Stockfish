...
// stable across multiple search iterations, we can quickly return the best
// move.
...
Value search(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth,
             bool cutNode, bool skipEarlyPruning);
...
void update_stats(const Position &pos, Stack *ss, Move move, Move *quiets,
    ...
    for (int d = 1; d < 64; ++d)
      for (int mc = 1; mc < 64; ++mc) {
        ...
        Reductions[PV][imp][d][mc] =
            std::max(Reductions[NonPV][imp][d][mc] - 1, 0);
...
/// Search::clear() resets search state to its initial value, to obtain
...
template <bool Root> uint64_t Search::perft(Position &pos, Depth depth) {
...
/// the UCI 'go' command. It searches from the root position and outputs the
/// "bestmove".
...
void MainThread::search() {
  ...
  if (!this->easyMovePlayed && Options["MultiPV"] == 1 && !Limits.depth &&
...
void Thread::search() {
  ...
  Stack stack[MAX_PLY + 7],
      *ss = stack + 4; // To allow referencing (ss-4) and (ss+2)
  ...
  for (int i = 4; i > 0; i--)
    (ss - i)->history =
        &this->counterMoveHistory[NO_PIECE][0]; // Use as sentinel
  ...
  // reached
  while ((rootDepth += ONE_PLY) < DEPTH_MAX && !Signals.stop &&
         !(Limits.depth && mainThread && rootDepth / ONE_PLY > Limits.depth)) {
    // Distribute search depths across the threads
    if (idx) {
      int i = (idx - 1) % 20;
      if (((rootDepth / ONE_PLY + rootPos.game_ply() + skipPhase[i]) /
           skipSize[i]) %
          2)
        continue;
    }
    ...
    for (PVIdx = 0; PVIdx < multiPV && !Signals.stop; ++PVIdx) {
      ...
      if (rootDepth >= 5 * ONE_PLY) {
        delta = Value(18);
        alpha =
            std::max(rootMoves[PVIdx].previousScore - delta, -VALUE_INFINITE);
      ...
      while (true) {
        bestValue =
            ::search<PV>(rootPos, ss, alpha, beta, rootDepth, false, false);
        ...
        //    (bestValue <= alpha || bestValue >= beta) && Time.elapsed() >
        //    3000)
    ...
    if (Limits.use_time_management()) {
      if (!Signals.stop && !Signals.stopOnPonderhit) {
        ...
        int improvingFactor =
            std::max(229, std::min(715, 357 + 119 * F[0] - 6 * F[1]));
        ...
        if (rootMoves.size() == 1 ||
            Time.elapsed() >
                Time.optimum() * unstablePvFactor * improvingFactor / 628 ||
...
Value search(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth,
  ...
  if (!rootNode) {
    ...
    if (Signals.stop.load(std::memory_order_relaxed) || pos.is_draw(ss->ply) ||
        ss->ply >= MAX_PLY)
    ...
    if (ttMove) {
      if (ttValue >= beta) {
        ...
        if ((ss - 1)->moveCount == 1 && !pos.captured_piece())
          update_cm_stats(ss - 1, pos.piece_on(prevSq), prevSq,
  ...
  if (inCheck) {
    ...
    eval = ss->staticEval = (ss - 1)->currentMove != MOVE_NULL
                                ? evaluate(pos)
  ...
  if (!PvNode && depth >= 5 * ONE_PLY && abs(beta) < VALUE_MATE_IN_MAX_PLY) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
      if (pos.legal(move)) {
        ss->currentMove = move;
        ss->history =
            &thisThread->counterMoveHistory[pos.moved_piece(move)][to_sq(move)];

        assert(depth >= 5 * ONE_PLY);
        pos.do_move(move, st);
        value = -search<NonPV>(pos, ss + 1, -rbeta, -rbeta + 1,
                               depth - 4 * ONE_PLY, !cutNode, false);
        pos.undo_move(move);
        if (value >= rbeta)
          return value;
      }
...
moves_loop: // When in check search starts from here
  ...
  improving = ss->staticEval >= (ss - 2)->staticEval
              /* || ss->staticEval == VALUE_NONE Already implicit in the
                 previous condition */
              || (ss - 2)->staticEval == VALUE_NONE;

  singularExtensionNode =
      !rootNode && depth >= 8 * ONE_PLY && ttMove != MOVE_NONE &&
      ttValue != VALUE_NONE &&
  ...
  // Loop through all pseudo-legal moves until no moves remain or a beta cutoff
  // occurs
  while ((move = mp.next_move(skipQuiets)) != MOVE_NONE) {
    ...
    //  sync_cout << "info depth " << depth / ONE_PLY << " currmove "
    ...
    givesCheck = type_of(move) == NORMAL && !pos.discovered_check_candidates()
                     ? pos.check_squares(type_of(pos.piece_on(from_sq(move)))) &
                           to_sq(move)
    ...
    // (alpha-s, beta-s), and just one fails high on (alpha, beta), then that
    // move is singular and should be extended. To verify this we do a reduced
    // search on all the other moves but the ttMove and if the result is lower
    // than ttValue minus a margin then we will extend the ttMove.
    ...
    if (!rootNode && pos.non_pawn_material(pos.side_to_move()) &&
      ...
      if (!captureOrPromotion && !givesCheck &&
        ...
        int lmrDepth =
            std::max(newDepth - reduction<PvNode>(improving, depth, moveCount),
                     DEPTH_ZERO) /
            ONE_PLY;
    ...
    if (depth >= 3 * ONE_PLY && moveCount > 1 &&
      ...
      if (captureOrPromotion)
        ...
        ss->statScore =
            cmh[moved_piece][to_sq(move)] + fmh[moved_piece][to_sq(move)] +
        ...
        r = std::max(DEPTH_ZERO,
                     (r / ONE_PLY - ss->statScore / 20000) * ONE_PLY);
      }
    ...
    if (doFullDepthSearch)
      value =
          newDepth < ONE_PLY
              ? givesCheck
                    ? -qsearch<NonPV, true>(pos, ss + 1, -(alpha + 1), -alpha)
  ...
  */
  ...
  else if (depth >= 3 * ONE_PLY && !pos.captured_piece() &&
...
Value qsearch(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth) {
  ...
  while ((move = mp.next_move()) != MOVE_NONE) {
    ...
    if ((!InCheck || evasionPrunable) && type_of(move) != PROMOTION &&
        !pos.see_ge(move))
      continue;
    ...
    if (value > bestValue) {
      ...
      if (value > alpha) {
        ...
        {
          tte->save(posKey, value_to_tt(value, ss->ply), BOUND_LOWER, ttDepth,
...
// update_stats() updates move sorting heuristics when a new quiet best move is
// found
...
string UCI::pv(const Position &pos, Depth depth, Value alpha, Value beta) {
  ...
  for (size_t i = 0; i < multiPV; ++i) {
    ...
    ss << "info"
       << " depth " << d / ONE_PLY << " seldepth " << pos.this_thread()->maxPly
...
void Tablebases::filter_root_moves(Position &pos,
  ...
  if (RootInTB && !UseRule50)
    TB::Score =
        TB::Score > VALUE_DRAW
            ...
            : TB::Score < VALUE_DRAW ? -VALUE_MATE + MAX_PLY + 1 : VALUE_DRAW;
}
