...
std::ostream &operator<<(std::ostream &os, Term t) {
  ...
  if (t == MATERIAL || t == IMBALANCE || t == Term(PAWN) || t == TOTAL)
    ...
    os << std::setw(5) << scores[t][WHITE][MG] << " " << std::setw(5)
       << scores[t][WHITE][EG] << " | " << std::setw(5) << scores[t][BLACK][MG]
...
const Score ThreatByMinor[PIECE_TYPE_NB] = {S(0, 0),   S(0, 33),   S(45, 43),
                                            S(46, 47), S(72, 107), S(48, 118)};

const Score ThreatByRook[PIECE_TYPE_NB] = {S(0, 0),   S(0, 25), S(40, 62),
                                           S(40, 59), S(0, 34), S(35, 48)};
...
const Value Passed[][RANK_NB] = {{V(5), V(5), V(31), V(73), V(166), V(252)},
                                 {V(7), V(14), V(38), V(73), V(166), V(252)}};

// PassedFile[File] contains a bonus according to the file of a passed pawn
const Score PassedFile[FILE_NB] = {S(9, 10),    S(2, 10), S(1, -8), S(-20, -12),
...
Score Evaluation<T>::evaluate_pieces() {
  ...
  while ((s = *pl++) != SQ_NONE) {
    ...
    if (Pt == ROOK) {
      ...
      if (relative_rank(Us, s) >= RANK_5)
        score += RookOnPawn *
                 popcount(pos.pieces(Them, PAWN) & PseudoAttacks[ROOK][s]);
...
const Bitboard KingFlank[FILE_NB] = {QueenSide,   QueenSide,   QueenSide,
                                     CenterFiles, CenterFiles, KingSide,
                                     KingSide,    KingSide};
...
template <Tracing T> template <Color Us> Score Evaluation<T>::evaluate_king() {
  ...
  const Square Up = (Us == WHITE ? NORTH : SOUTH);
  const Bitboard Camp =
      (Us == WHITE ? ~Bitboard(0) ^ Rank6BB ^ Rank7BB ^ Rank8BB
  ...
  if (kingAttackersCount[Them] > (1 - pos.count<QUEEN>(Them))) {
    ...
    kingOnlyDefended =
        attackedBy[Them][ALL_PIECES] & attackedBy[Us][KING] & ~attackedBy2[Us];
    ...
    kingDanger = kingAttackersCount[Them] * kingAttackersWeight[Them] +
                 ...
                 848 * !pos.count<QUEEN>(Them) - 9 * mg_value(score) / 8 + 40;

    // Analyse the safe enemy's checks which are possible on next move
    safe = ~pos.pieces(Them);
    safe &=
        ~attackedBy[Us][ALL_PIECES] | (kingOnlyDefended & attackedBy2[Them]);
    ...
    safe |= attackedBy2[Them] & ~(attackedBy2[Us] | pos.pieces(Them)) &
            attackedBy[Us][QUEEN];
    ...
    // Transform the kingDanger units into a Score, and substract it from the
    // evaluation
    if (kingDanger > 0)
...
Score Evaluation<T>::evaluate_threats() {
  ...
  b = (shift<Left>(b) | shift<Right>(b)) & pos.pieces(Them) &
      ~attackedBy[Us][PAWN];
...
template <Tracing T> template <Color Us> Score Evaluation<T>::evaluate_space() {
  ...
  int bonus =
      popcount((Us == WHITE ? safe << 32 : safe >> 32) | (behind & safe));
...
ScaleFactor Evaluation<T>::evaluate_scale_factor(Value eg) {
  ...
  if (sf == SCALE_FACTOR_NORMAL || sf == SCALE_FACTOR_ONEPAWN) {
    if (pos.opposite_bishops()) {
      // Endgame with opposite-colored bishops and no other pieces (ignoring
      // pawns) is almost a draw, in case of KBP vs KB, it is even more a draw.
      if (pos.non_pawn_material(WHITE) == BishopValueMg &&
          pos.non_pawn_material(BLACK) == BishopValueMg)
        return more_than_one(pos.pieces(PAWN)) ? ScaleFactor(31)
                                               : ScaleFactor(9);
    ...
    // pawns are drawish.
    else if (abs(eg) <= BishopValueEg && pos.count<PAWN>(strongSide) <= 2 &&
...
template <Tracing T> Value Evaluation<T>::value() {
  ...
  if (pos.non_pawn_material() >= SpaceThreshold)
    score += evaluate_space<WHITE>() - evaluate_space<BLACK>();
  ...
  if (T) {
    ...
    if (pos.non_pawn_material() >= SpaceThreshold)
      Trace::add(SPACE, evaluate_space<WHITE>(), evaluate_space<BLACK>());
    Trace::add(TOTAL, score);
  }

  return (pos.side_to_move() == WHITE ? v : -v) +
         Eval::Tempo; // Side to move point of view
}
...
std::string Eval::trace(const Position &pos) {
  ...
  ss << std::showpoint << std::noshowpos << std::fixed << std::setprecision(2)
     ...
     << "        Knights | " << Term(KNIGHT) << "        Bishops | "
     << Term(BISHOP) << "          Rooks | " << Term(ROOK)
     << "         Queens | " << Term(QUEEN) << "       Mobility | "
     << Term(MOBILITY) << "    King safety | " << Term(KING)
     << "        Threats | " << Term(THREAT) << "   Passed pawns | "
     << Term(PASSED) << "          Space | " << Term(SPACE)
     << "----------------+-------------+-------------+-------------\n"
     << "          Total | " << Term(TOTAL);
