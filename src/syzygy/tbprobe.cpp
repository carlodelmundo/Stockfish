...
int dtz_before_zeroing(WDLScore wdl) {
  return wdl == WDLWin
             ...
             : wdl == WDLCursedWin
                   ? 101
                   : wdl == WDLBlessedLoss ? -101 : wdl == WDLLoss ? -1 : 0;
}
...
struct PairsData {
  ...
  uint16_t *blockLength;           // Number of stored positions (minus one) for each
                                   // block: 1..65536
  int blockLengthSize;             // Size of blockLength[] table: padded so it's bigger
                                   // than blocksNum
...
struct Atomic {
  Atomic() = default;
  Atomic(const Atomic &e) {
    ready = e.ready.load();
  } // MSVC 2013 wants assignment within body
...
auto item(WDLPieceTable &e, int stm, int) -> decltype(e[stm]) & {
  return e[stm];
}
auto item(DTZPieceTable &e, int, int) -> decltype(e) & { return e; }
auto item(WDLPawnTable &e, int stm, int f) -> decltype(e.file[stm][f]) & {
  return e.file[stm][f];
}
auto item(DTZPawnTable &e, int, int f) -> decltype(e.file[f]) & {
  return e.file[f];
}
...
const Value WDL_to_value[] = {-VALUE_MATE + MAX_PLY + 1, VALUE_DRAW - 2,
...
public:
  ...
  uint8_t *map(void **baseAddress, uint64_t *mapping, const uint8_t *TB_MAGIC) {
    ...
    *baseAddress = MapViewOfFile(mmap, FILE_MAP_READ, 0, 0, 0);
    ...
    if (*data++ != *TB_MAGIC++ || *data++ != *TB_MAGIC++ ||
        *data++ != *TB_MAGIC++ || *data++ != *TB_MAGIC) {
...
WDLScore map_score(WDLEntry *, File, int value, WDLScore) {
...
int map_score(DTZEntry *entry, File f, int value, WDLScore wdl) {
  ...
  uint8_t *map = entry->hasPawns ? entry->pawnTable.map : entry->pieceTable.map;
...
T do_probe_table(const Position &pos, Entry *entry, WDLScore wdl,
                 ProbeState *result) {
  ...
  if (entry->hasPawns) {
    ...
    Piece pc =
        Piece(item(entry->pawnTable, 0, 0).precomp->pieces[0] ^ flipColor);
...
template <typename T> uint8_t *set_dtz_map(WDLEntry &, T &, uint8_t *, File) {
  return nullptr;
}
...
template <typename Entry> void *init(Entry &e, const Position &pos) {
  ...
  uint8_t *data =
      TBFile(fname).map(&e.baseAddress, &e.mapping, TB_MAGIC[IsWDL]);
  if (data)
...
void Tablebases::init(const std::string &paths) {
  ...
  sync_cout << "info string Found " << EntryTable.size() << " tablebases"
            << sync_endl;
}
