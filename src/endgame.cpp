...
template <> Value Endgame<KBNK>::operator()(const Position &pos) const {
  ...
  Value result = VALUE_KNOWN_WIN + PushClose[distance(winnerKSq, loserKSq)] +
...
template <> Value Endgame<KRKP>::operator()(const Position &pos) const {
  ...
  // the position is drawish
  else if (rank_of(bksq) <= RANK_3 && distance(bksq, psq) == 1 &&
           rank_of(wksq) >= RANK_4 &&
    ...
    result = Value(200) -
             8 * (distance(wksq, psq + SOUTH) - distance(bksq, psq + SOUTH) -
...
template <> ScaleFactor Endgame<KBPsK>::operator()(const Position &pos) const {
  ...
  if ((pawnsFile == FILE_A || pawnsFile == FILE_H) &&
    ...
    Square bishopSq = pos.square<BISHOP>(strongSide);
    Square queeningSq =
        relative_square(strongSide, make_square(pawnsFile, RANK_8));
...
template <> ScaleFactor Endgame<KRPKR>::operator()(const Position &pos) const {
  ...
  if (r == RANK_6 && distance(bksq, queeningSq) <= 1 &&
  ...
  // away, it's a draw.
  if (r <= RANK_5 && bksq == wpsq + NORTH &&
      distance(wksq, wpsq) - tempo >= 2 && distance(wksq, brsq) - tempo >= 2)
    return SCALE_FACTOR_DRAW;
  ...
  if (f != FILE_A && file_of(wrsq) == f && wrsq < wpsq &&
      (distance(wksq, queeningSq) < distance(bksq, queeningSq) - 2 + tempo) &&
      (distance(wksq, wpsq + NORTH) <
       distance(bksq, wpsq + NORTH) - 2 + tempo) &&
      (distance(bksq, wrsq) + tempo >= 3 ||
       (distance(wksq, queeningSq) < distance(bksq, wrsq) + tempo &&
        (distance(wksq, wpsq + NORTH) < distance(bksq, wrsq) + tempo))))
    return ScaleFactor(SCALE_FACTOR_MAX - 8 * distance(wpsq, queeningSq) -
  ...
  if (r <= RANK_4 && bksq > wpsq) {
    ...
    if (distance<File>(bksq, wpsq) == 1 && distance(wksq, bksq) > 2)
...
ScaleFactor Endgame<KRPPKRP>::operator()(const Position &pos) const {
  ...
  if (distance<File>(bksq, wpsq1) <= 1 && distance<File>(bksq, wpsq2) <= 1 &&
  ...
  case 1:
    ...
    if (ksq == blockSq1 && opposite_colors(ksq, wbsq) &&
      ...
      return SCALE_FACTOR_DRAW;
             ...
             (bbsq == blockSq1 || (pos.attacks_from<BISHOP>(blockSq1) &
...
/// and the defending king prevents the pawn from advancing, the position is
/// drawn.
...
template <> ScaleFactor Endgame<KPKP>::operator()(const Position &pos) const {
  ...
  return Bitbases::probe(wksq, psq, bksq, us) ? SCALE_FACTOR_NONE
                                              : SCALE_FACTOR_DRAW;
}
