...
template <>
PieceType min_attacker<KING>(const Bitboard *, Square, Bitboard, Bitboard &,
                             Bitboard &) {
...
Position &Position::set(const string &fenStr, bool isChess960, StateInfo *si,
  ...
  /*
     ...
     move, this is the position "behind" the pawn. This is recorded only if
     there is a pawn in position to make an en passant capture, and if there
     really is a pawn that might have advanced two squares.
...
void Position::set_check_info(StateInfo *si) const {
  ...
  si->blockersForKing[WHITE] = slider_blockers(
      pieces(BLACK), square<KING>(WHITE), si->pinnersForKing[WHITE]);
  si->blockersForKing[BLACK] = slider_blockers(
      pieces(WHITE), square<KING>(BLACK), si->pinnersForKing[BLACK]);
...
const string Position::fen() const {
  ...
  if (can_castle(WHITE_OO))
    ss << (chess960
               ? char('A' + file_of(castling_rook_square(WHITE | KING_SIDE)))
               : 'K');

  if (can_castle(WHITE_OOO))
    ss << (chess960
               ? char('A' + file_of(castling_rook_square(WHITE | QUEEN_SIDE)))
               : 'Q');

  if (can_castle(BLACK_OO))
    ss << (chess960
               ? char('a' + file_of(castling_rook_square(BLACK | KING_SIDE)))
               : 'k');

  if (can_castle(BLACK_OOO))
    ss << (chess960
               ? char('a' + file_of(castling_rook_square(BLACK | QUEEN_SIDE)))
               : 'q');
...
/// Position::game_phase() calculates the game phase interpolating total
...
Phase Position::game_phase() const {
  ...
  return Phase(((npm - EndgameLimit) * PHASE_MIDGAME) /
...
bool Position::see_ge(Move m, Value threshold) const {
  ...
  while (true) {
    ...
    balance +=
        relativeStm ? PieceValue[MG][nextVictim] : -PieceValue[MG][nextVictim];
...
bool Position::is_draw(int ply) const {
  ...
  for (int i = 4; i <= end; i += 2) {
    ...
    if (stp->key == st->key && ++cnt + (ply - 1 > i) == 2)
      return true;
  }
